...
| `PORKBOT_COMPUTER_MEMORY_MB`      | optional | `512`     | One bot's memory ceiling in MB. The default is a shell-shaped sandbox's share; `2048` is what a browser-inside-the-sandbox run needs. Raise it for the deployment here, or for one bot through the provider's `ceilings` override. Raising it raises every bot's share at once — the memory floor is this value times the bots active at once — so a thoughtless raise outgrows the host. |
| `PORKBOT_COMPUTER_SWAP_MB`        | optional | `256`     | One bot's swap bound in MB, beyond `PORKBOT_COMPUTER_MEMORY_MB`; `0` disables swap. Kept smaller than the memory ceiling so a runaway bot pages a little and dies instead of filling the host's swap. Raising it toward the memory ceiling is how a runaway gets to thrash the host before it dies.                                                                                       |
| `PORKBOT_COMPUTER_DISK_MB`        | optional | `10240`   | One bot's disk budget in MB. The floor here scales with every _configured_ bot, because a home volume survives a park.                                                                                                                                                                                                                                                                    |
| `PORKBOT_COMPUTER_IDLE_MS`        | optional | `180000`  | How long a machine may go without a command before it is parked; `0` disables the sweep. The home survives a park and the memory returns to the host. Raising it keeps a quiet bot's memory on the host for longer: the park window is what makes an idle fleet cost disk rather than RAM.                                                                                                |
...
| `PORKBOT_COMPUTER_DISK_QUOTA`            | optional | `auto`                                 | `auto`, `none` or `storage-opt`. `auto` detects the daemon's storage driver and enforces `PORKBOT_COMPUTER_DISK_MB` where it answers (`btrfs`, or `overlay2` over xfs with `pquota`), reporting an explicit no-op where it does not. |
| `PORKBOT_COMPUTER_SNAPSHOT_KEEP`         | optional | `10`                                   | How many snapshot captures one bot keeps; older archives are pruned after a capture and once at supervisor boot. Zero keeps every capture.                                                                                           |
