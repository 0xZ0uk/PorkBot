...
- **A computer is offline, Docker or a cloud sandbox.** `supervisor` owns the
  ...
  `PORKBOT_COMPUTER_CPUS` / `PORKBOT_COMPUTER_MEMORY_MB` are one bot's measured
  share, and `PORKBOT_COMPUTER_DISK_MB` is a write-layer budget enforced only
  where the daemon's storage driver answers it, under "A
