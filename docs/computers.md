...
- **Sizing.** `PORKBOT_COMPUTER_CPUS` (default `1`),
  ...
  the host. The memory default is a shell-shaped sandbox's share — the
  measured floor records one in the tens of megabytes — and `2048` is what a
  ...
  is `PORKBOT_COMPUTER_PIDS` (default `512`). The three terms cost different
